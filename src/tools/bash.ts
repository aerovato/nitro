...
- Prefer the final command that fulfills the request. Chain with && only when steps must be sequential.
- No preflight: do not ls/which/type/command -v or test existence before an obvious convert/move/edit.
- Do not probe whether a tool exists. Run the real command; on failure, read the error and retry or report.
- Explore only when the correct command depends on unknown state (collisions, unknown layout). Never "just check."
